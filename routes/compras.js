...
// GET todas las compras con nombres
...
router.get('/:id', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM compras WHERE id_compra = ?', [req.params.id]);
...
// POST crear compra con detalle (transacción)
router.post('/', async (req, res) => {
  ...
  // Calcular total
  const total = herramientas.reduce((sum, h) => sum + (parseFloat(h.precio_unitario) * parseInt(h.cantidad)), 0);
  ...
  try {
    ...
    // 2. Crear detalle (esto activa el trigger que SUBE el stock automáticamente)
    ...
    await conn.commit();
    res.json({ success: true, message: 'Compra registrada', id: id_compra, total });
  } catch (err) {
    await conn.rollback();
    res.status(500).json({ success: false, error: err.message });
  } finally {
